...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/20 19:59:53 by lfrancav &        #+#    #+#             */
/*   Updated: 2024/02/20 19:59:53 by lfrancav &       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parser_checks(t_expression *tmp, t_expression **exps)
{
	...
	if (tmp->type == OPERATOR && tmp->next->type == OPERATOR)
		return (parse_error_near(tmp->value));
	if (tmp == *exps && (tmp->type == OPERATOR || tmp->type == PARENT_CL))
		return (parse_error_near(tmp->value));
	if (tmp->type == PARENT_OP && tmp->next && tmp->next->type == PARENT_CL)
		return (parse_error_near(tmp->value));
	return (OK);
}
